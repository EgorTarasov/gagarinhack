...
async def upload_achievement(
    ...
    minio_client.put_object(
        ...
        object_name=static_name,
        data=file.file,
        length=file.size,
    )
    new_id = await crud.create(db_conn, achievement, static_name)
    return new_id


async def download_achievements(
    db_conn: PoolConnectionProxy, user_id: int, offset: int = 0, limit: int = 100
) -> list[AchievementDto]:
    return await crud.get_all(db_conn, user_id=user_id, offset=offset, limit=limit)


async def get_achievement(
    db_conn: PoolConnectionProxy, achievement_id: int
) -> AchievementDto:
