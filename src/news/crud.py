...
async def insert_all(db: PoolConnectionProxy, all_news: list[NewsDao]):
    ...
    for news in all_news:
        query = 'INSERT INTO "news" (title, created_at, description, image_link) VALUES ($1, $2, $3, $4) RETURNING id'
        _id = await db.fetchval(query, news.title, news.created_at, news.description, news.image_link)
        print(_id)
